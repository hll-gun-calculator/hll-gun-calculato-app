...
class CalcUtil {
  CalcResult on({
    ...
    if (calculatingFunctionInfo.child!.isEmpty) {
      result = CalcResultStatus(
          message: "此${calculatingFunctionInfo.name}函数包没有配置任何阵营函数", code: 1101);
      return CalcResult(result: result);
    }

    if (calculatingFunctionInfo.child!.isEmpty &&
        calculatingFunctionInfo.child!.containsKey(inputFactions.value)) {
      result = CalcResultStatus(
          message:
              "此${calculatingFunctionInfo.name}函数包没有配置${inputFactions.value}函数公式，无法计算结果",
          code: 1102);
      return CalcResult(result: result);
    }

    if (calculatingFunctionInfo.child![inputFactions.value]["fun"] == "") {
      result = CalcResultStatus(
          message:
              "此${calculatingFunctionInfo.name}函数包配置${inputFactions.value}函数公式无效",
          code: 1103);
      return CalcResult(result: result);
    }
