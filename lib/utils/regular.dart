...
class Regular {
  ...
  /// 正则验证
  RegularResult check(RegularType regularType, dynamic value) {
    if (regularType == RegularType.None) return RegularResult(code: -1);
    final checkResult = REGULARTYPE[regularType]!.v.hasMatch(value);

    if (checkResult) {
      return RegularResult(code: 0, checkResult: checkResult);
    }

    return RegularResult(code: -1);
  }
